...
> 本手册版本：2026-05-03 v3
> 配套文件：
> - `18个投资人提示词-RikkaHub版.md` — 18个提示词（系统提示词+用户模板）
> - `RikkaHub导入格式/系统提示词-MD版/` — 18个纯 MD，直接复制到提示词框
> - `RikkaHub导入格式/模式注入/` — 6个全局规则（正确 JSON 格式）
> - `RikkaHub导入格式/提示词注入-18个投资人技能/` — 18个技能注入（正确 JSON 格式）
> - `RikkaHub导入格式/世界书-投资人关键词触发.json` — 6个关键词触发（正确 lorebook 格式）
> - GitHub：`acheng-byte/skills-library`（可直接导入 Agent Skills）
