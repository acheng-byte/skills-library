...
> 国内直接用会报 `Failed to connect to raw.githubusercontent.com`，开 VPN 后可用。

1. 进入扩展管理 → Agent Skills
2. 点右下角 ↓（导入）按钮
3. 输入完整 GitHub 仓库 URL：

```
https://github.com/acheng-byte/skills-library
```
...
单个技能（如死亡笔记）：
```
https://github.com/acheng-byte/skills-library/tree/master/skills/08-项目死亡笔记
```
...
>
> Agent Skills（GitHub 导入）：
> - 总控：`acheng-byte/skills-library`
> - 单技能示例：`acheng-byte/skills-library/tree/master/skills/08-项目死亡笔记`
